...
const CreatePin = ({ user }) => {
  ...
  const uploadImage = (e) => {
    ...
    if(type === 'image/png' || type === 'image/svg' || type === 'image/jpeg'  || type === 'image/gif'  || type === 'image/tiff') {
      ...
      client.assets
        .upload('image', e.target.files[0], { contextType: type, filename: name })
        .then((document) => {
    ...
    <div className="flex flex-col justify-center items-center mt-5 lg:h-4/5">
      ...
      <div className="flex lg:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full">
            ...
            {!imageAsset ? (
              <label>
                <div className="flex flex-col items-center justify-center h-full">
                  <div className="flex flex-col justify0center items-center">
              ...
              <div className="relative h-full">
                ...
                <button onClick={() => setImageAsset(null)} type="button" className="absolute bottom-3 right-3 p-3 rounded-full bg-white text-xl cursor-pointer outline-none hover:shadow-md transition-all duration-500 ease-in-out">
                  <MdDelete />
                </button>
              </div>
            )}
        ...
        <div className="flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full">
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Add title here" className="outline-none text-base sm:text-lg border-b-2 border-gray-200 p-2" />
          {user && (
          ...
          <input type="text" value={about} onChange={(e) => setAbout(e.target.value)} placeholder="Add what is your pin about" className="outline-none text-base sm:text-lg border-b-2 border-gray-200 p-2" />
          ...
          <div className="flex flex-col">
            <div>
              ...
              <select onChange={(e) => setCategory(e.target.value)} className="outline-non w-4/5 text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
