...
const Pin = ({ pin: { postedBy, image, _id, destination, save } }) => {
  ...
  return (
    <div className="m-2">
      ...
      >
        <img src={urlFor(image).width(250).url()} className="rounded-lg w-full" alt="User pin" />
        {postHovered && (
          <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50" style={{ height: '100%' }}>
            <div className="flex items-center justify-between">
              ...
              {alreadySaved ? (
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); savePin(_id)}} className="bg-red-500 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none">
                  Save
                </button>
              )}
            </div>
